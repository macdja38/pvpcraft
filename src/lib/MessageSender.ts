...
class MessageSender {
  ...
  private _queuedMessages: Map<string | Eris.TextChannel, { channel: Eris.TextChannel | string, text: string, options: { attachments: any[] }, }[]>;
  ...
  emptyQueue() {
    ...
    for (let [channelId, queue] of queuedMessages) {
      ...
      this._getWebhook(calledOptions.channel).then(webhook => {
        ...
        const calledOptionsChannel = calledOptions.channel;
        ...
        texts.forEach((string) => {
          calledOptionsChannel.createMessage(this.translate(calledOptionsChannel.id)`${string}\nPlease give the bot \"Manage Webhooks\" to enable non fallback functionality`).catch(() => {
          });
        });
      })
    }
   ...
   */
  sendQueuedMessage(channel: Eris.TextChannel | string, text: string, options: any) {
    const key = (channel instanceof Eris.TextChannel) ? channel.id : channel;
    const queue = this._queuedMessages.get(key) || []
    queue.push({ channel, text, options })
    this._queuedMessages.set(key, queue);
  }
