...
class Player extends EventEmitter {
  ...
  canInit(voice) {
    return voice.permissionsOf(this.client.user.id).has("voiceConnect") &&
      ...
      voice.permissionsOf(this.client.user.id).has('readMessages');
  }
  ...
  _logQueueError(command, error) {
    ...
    const stringError = splitError == null ? error.message : `${splitError[0]}...`;
   ...
   */
  async shuffle() {
    await this.musicDB.shuffle(this.guild.id);
