...
class Utils {
  ...
  static resolveAllPromises(array, timeout) {
    return new Promise((resolve, reject) => {
      ...
      const promiseCatch = function(error) {
        remaining -= 1;
        if (remaining === 0) process.nextTick(resolveWithResults);
        throw error;
      };
      const promiseThen = function(value) {
        remaining -= 1;
        if (remaining === 0) process.nextTick(resolveWithResults);
        return value;
      };
   ...
   */
  static handleErisRejection(promise) {
    Utils._logPromiseError(promise).catch(console.error);
  }

  static async _logPromiseError(promise) {
    try {
      await promise
    } catch (error) {
      console.error(error);
   ...
   * @returns {Request}
   */
  static mediaWikiSearch(api, searchTerm) {
    return request.get(`${api}?action=opensearch&search=${encodeURIComponent(searchTerm)}`).then(result => JSON.parse(result));
  }
