...
class PvPCraft {
  ...
  private moduleList: ModuleWrapper[];
  private middlewareList: MiddlewareWrapper[];
  ...
  private git?: { commit: string, branch: string };
  ...
  for (let module of modulesAndMiddleware) {
    for (let command of module.commands || module) {
      ...
      if (command.subCommands && userCommand.args.length > 0) {
        const moduleWrappers: ModuleWrapper[] = "module" in module ? [{ commands: command.subCommands, module: module.module }] : [];
        const middlewareWrappers: MiddlewareWrapper[] = "ware" in module ? [{ commands: command.subCommands, ware: module.ware }] : [];
