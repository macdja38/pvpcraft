  ...
  getCommands(): ModuleCommand[] {
    return [{
      ...
      execute: (command: Command) => {
        return command.replyAutoDeny(command.translate `Usage ${command.prefix}${command.command} <add|remove|list|join|leave>`)
      },
      subCommands: [
        {
          triggers: ["add"],
          ...
          execute: (command: Command) => {
            ...
            if (command.options.role) {
              ...
              if (roleMatch) {
                roleId = command.channel.guild.roles.get(roleMatch[1]);
              }
              else {
          ...
          execute: (command: Command) => {
            ...
            if (role) {
              command.channel.guild.addMemberRole(command.author.id, role.id).then(() => {
                ...
                if (error) {
                  command.replyAutoDeny(command.translate `Error ${error.toString()} promoting ${utils.removeBlocks(command.author.username)} try making sure the bot's highest role is above the role you want it to add and that the bot has Manage Permissions or Admin.`)
                }
              });
            } else {
          ...
          execute: (command: Command) => {
            ...
            if (role) {
              command.channel.guild.removeMemberRole(command.author.id, role.id).then(() => {
                ...
                command.createMessageAutoDeny(command.translate `${error.toString()} demoting ${utils.removeBlocks(command.author.username)} try redefining your rank and making sure the bot has enough permissions.`).catch(console.error)
              })
            } else {
              command.replyAutoDeny(command.translate `Role could not be found, have an administrator use \`${command.prefix}rank add\` to update it.`);
              return true;
