        ...
        Promise.resolve(P.getPokemonByName(pokemon_name.toLowerCase())).then((response) => {
          ...
          .catch(function (error) {
              if (error.statusCode == "404") {
                msg.reply("Could not find **" + utils.clean(command.args[0]) + "**").catch(perms.getAutoDeny(msg));
              } else {
                console.log('There was an ERROR with getting the data: ', error);
                msg.reply("Error getting data " + error).catch(perms.getAutoDeny(msg));
              }
            }
          );
      }
    ...
    if (command.commandnos === "pokestat" && perms.check(msg, "pokemon.pokestat")) {
      ...
      if (pokemon_name && /^[^<@#\\\/>]*$/g.test(pokemon_name)) {
        Promise.resolve(P.getPokemonByName(pokemon_name.toLowerCase())).then((response) => {
          ...
          .catch(function (error) {
            if (error.statusCode == "404") {
              msg.reply("Could not find " + command.args[0]).catch(perms.getAutoDeny(msg));
            } else {
              console.log('There was an ERROR with getting the data: ', error);
              msg.reply("Error getting data " + error).catch(perms.getAutoDeny(msg));
            }
        ...
        msg.reply("Sorry invalid input").catch(perms.getAutoDeny(msg));
      }
      return true;
