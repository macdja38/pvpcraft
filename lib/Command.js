...
class Command {
   ...
   * @param {Role} [options.role] Role mentioned by command
   * @param {Channel} [options.channel] Channel mentioned by command
   * @param {User} [options.user] User mentioned by command
   ...
   * @private
   */
  static _isValidCommandType(prefix, message, options) {
    let insen = message.content.trim().toLowerCase();
    let m = message.content;
    for (let i in prefix) {
      if (prefix.hasOwnProperty(i) && insen.indexOf(prefix[i].toLowerCase()) === 0) {
        m = m.substr(prefix[i].length);
        return {prefix: utils.clean(prefix[i]), content: m};
      }
    }
  ...
  replyAutoDeny(content, file) {
    return this.capturePromiseRejection(this._reply(content, file).catch(this.autoDenyPermission.bind(this)), content);
  }
  ...
  createMessageAutoDeny(content, file) {
    return this.capturePromiseRejection(this.msg.channel.createMessage(content, file).catch(this.autoDenyPermission.bind(this)), content);
  }
