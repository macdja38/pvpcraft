...
class Player extends EventEmitter {
  ...
  destroy() {
    this.queue = [];
    if (this.connection) {
      this.connection.removeListener("end", this.playNext);
      this.connection.removeListener("error", this.playNext);
  ...
  init() {
    return new Promise((resolve, reject) => {
      ...
      this.client.joinVoiceChannel(this.voice.id).then((connection) => {
        ...
        connection.on('end', this.playNext);
        connection.on('error', this.playNext);
        resolve(connection);
      }).catch(error => {
   ...
   * @param {Command} command PvPCraft command object
   * @param {string[]} args arguments to queue
   * @param {number?} limit max songs to queue
   */
  enqueue(msg, command, args, limit = 0) {
    let currentUserQueueLength = this.getUserVidCount(msg.author);
    let nowQueued = 0;
    if (limit === currentUserQueueLength) {
      command.replyAutoDeny(`Sorry you have reached your limit of ${limit} songs in the queue, Please try again after some have been played`);
      return;
    }
    let text = args;
    text = text.join(" ");
    ...
    for (let song of text) {
      this.resolveVid(msg, command, song, this.key, (vid) => {
        if (!vid) return;
        ...
        if (vid.video) {
          ...
          }).catch(this._logQueueError.bind(this, msg, command));
        }
        else if (vid.playlist) {
          if (this.queueingPlaylist) {
            command.replyAutoDeny("Sorry only one playlist can be queued at a time.");
            return true;
          }
          if (!this.key) {
            command.replyAutoDeny("Playlists disabled. (No api key)");
            return true;
          }
   ...
   * @private
   */
  _logQueueError(msg, command, error) {
    console.log(error);
  ...
  playNext(possibleError) {
    ...
    if (this.connection) {
      ...
      this.text.createMessage(`Playing ${this.currentVideo.prettyPrint()}${possibleError ? `\nLast song skipped because of ${possibleError}`:""}`);
    }
  }

  resolveVid(msg, command, targetVideo, key, callback) {
    ...
    if (/^http/.test(targetVideo)) {
      ...
      command.replyAutoDeny("Not a youtube video.");
      callback(false);
    }
    else {
      ...
      request(requestUrl, (error, response) => {
        if (!error && response.statusCode == 200) {
          ...
          if (body.items.length == 0) {
            command.replyAutoDeny('Your query gave 0 results.');
            callback(false);
            return;
          }
          ...
          command.replyAutoDeny('No video has been found!');
          callback(false);
        } else {
          command.replyAutoDeny('There was an error searching.');
          callback(false);
        }
      });
    }
