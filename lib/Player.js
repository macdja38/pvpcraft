...
class Player extends EventEmitter {
  ...
  init() {
    return new Promise((resolve, reject) => {
      console.log("Trying to resume into " + this.voice.name);
  ...
  async playNext(from, possibleError) {
    ...
    if (this.connection) {
      ...
      if (source.container === "webm" && source.encoding === "opus"
        ...
        this.slowSender.sendMessage(this.text,`Skipping ${videoUtils.prettyTitle(this.currentVideoInfo)} because it's not an easy format to play`);
