...
const YoutubeTrack = require("./youtube-track");
...
class Player extends EventEmitter {
   ...
   * @param {number} [limit=0] optional limit for songs the member can have queued
   */
  enqueue(text, member, command, limit = 0) {
    ...
    thingList.forEach((song) => {
      this.resolveVid(song).then((video) => {
        if (video.hasOwnProperty("playlist")) {
          this.enqueuePlaylist(video.playlist, member, command, limit);
        } else if (video.hasOwnProperty("video")) {
          this.enqueueSong(video.video, member, command);
        }
      });
    })
  }
  ...
  async enqueuePlaylist(playlist, member, command, limit) {
    let currentUserQueueLength = await this.getUserVidCount(member);
    let requestLimit = this.premium ? 250 : 50;
    if (limit) {
      requestLimit = Math.min(limit - currentUserQueueLength, requestLimit);
    }
  ...
  async prettyList() {
    ...
    for (; i < Math.min(songList.length, 10); i++) {
      ...
      table.push([
        ...
        videoUtils.prettyVotes(this.currentVideo),
  ...
  async playNext(from, possibleError) {
    console.log("playNext Called from", from, " ", possibleError);
    this.lastPlay = Date.now();
    if (possibleError) {
      if (this.raven) {
    ...
    if (this.connection) {
      ...
      let options = {frameDuration: 20, voiceDataTimeout: 10000};
      if (source.container === "webm" && source.encoding === "opus"
        || source.container === "ogg" && source.encoding === "opus") {
        options.format = source.container;
      } else {
        this.playNext("playNext2", "Not an easy to play format");
        return;
      }
      this.startTime = Date.now();
      process.nextTick(() => {
        try {
          console.log("calling connection.play with", source, options);
          this.currentOptions = options;
          this.connection.play(source.url/* source.url */, options);
        } catch (error) {
      ...
      this.slowSender.sendMessage(this.text, `Playing ${videoUtils.prettyPrint(this.currentVideoInfo)} Container:${source.container} Encoding:${source.encoding}${possibleError ? `\nLast song skipped because of ${possibleError}` : ""}`);
    }
  }
  ...
  async resolveVid(text) {
    ...
    if (/^http/.test(text)) { // text is a URL
      ...
      let requestUrl = 'https://www.googleapis.com/youtube/v3/search' +
        `?part=snippet&q=${text}&key=${this.key}&regionCode=${this.regionCode}`;
      let response;
      try {
