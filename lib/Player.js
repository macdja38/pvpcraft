...
class Player extends EventEmitter {
  ...
  init() {
    return new Promise((resolve, reject) => {
      this._maybeLog(`Trying to init into ${this.voice.name}`);
      ...
      this.client.joinVoiceChannel(this.voice.id).then((connection) => {
        this._maybeLog(`Joined ${this.voice.name}`);
        this.ready = true;
        ...
        this._maybeLog(`Failed to join ${this.voice.name}`);
        reject(error);
      });
    })
  }
  ...
  async playNext(from, possibleError) {
    ...
    if (this.connection) {
      ...
      try {
        ...
        let url;
        if (!source.url.includes("ratebypass")) {
