...
class Player extends EventEmitter {
  ...
  _logQueueError(command, error) {
    const splitError = error.toString().match(/YouTube said: ((?:.|\n){5,500})/);
    let stringError;
    if (splitError != null) {
      stringError = splitError[0];
      stringError += "...";
    } else {
      stringError = error.toString().match(/.{0,1800}/);
      if (stringError !== null) {
        stringError = stringError[0];
      }
    }
    command.replyAutoDeny(this.i10010n() `Sorry while fetching that I encountered the error: ${utils.clean(stringError)}.`);
  }
  ...
  skipSong() {
    this.lastPlay = Date.now();
    if (this.connection.playing === false) {
  ...
  prettyQueue() {
    ...
    for (let song in this.queue) {
      if (this.queue.hasOwnProperty(song)) {
        table.push(
  ...
  async prettyList() {
    ...
    let [queue, queueLength] = await Promise.all([this.musicDB.getNextVideos(this.guild.id, 10), this.musicDB.countVideosQueued(this.guild.id)]);
    ...
    let resultTable = table.toString().replace(/\[(?:90|39)m/g, "");
    if (queueLength > 9) {
      const remainingLength = queueLength - 9;
  ...
  async playNext(from, possibleError) {
    ...
    if (this.connection) {
      ...
      try {
        ...
        this.playWhenReady(url, options, 20);
      } catch (error) {
  ...
  playWhenReady(url, options, tries) {
    try {
      this.connection.play(url, options);
    } catch (error) {
      if (error.toString() === "Not ready yet") {
        if (tries <= 0) {
          if (this.raven) this.raven.captureException(error, {
            extra: {
              options,
        ...
        setTimeout(this.playWhenReady.bind(this, url, options, tries - 1), 500)
      } else {
        if (this.raven) this.raven.captureException(error, {
          extra: {
            options,
