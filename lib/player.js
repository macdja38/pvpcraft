...
module.exports = class player {
    ...
    prettyList() {
        var table = new Table({
            head: ["#", "song", "author", "views", "length", "skip", "added by"],
            colWidths: [4, 40, 16, 13, 10, 6, 24],
        ...
        table.push(
            [0, this.currentVideo.prettyTitle(), this.currentVideo.prettyAuthor(), this.currentVideo.prettyViews(), this.currentVideo.prettyTime(),this.currentVideo.votes.length, utils.fullName(this.currentVideo.user)]
        );
        for (var song in this.queue) {
            ...
            table.push(
                [(parseInt(song) + 1), this.queue[song].prettyTitle(), this.queue[song].prettyAuthor(), this.queue[song].prettyViews(), this.queue[song].prettyTime(),this.queue[song].votes.length, utils.fullName(this.queue[song].user)]
            );
        }
        return "```xl\n" + table.toString().replace(/\[(?:90|39)m/g, "") + (this.queue.length>13 ? "\n ..." : "") +"```";
    }
