...
class player {
            ...
            if (!error && response.statusCode == 200) {
              ...
              body.items.forEach((elem, idx) => {
                ...
                promises.push(delayedPromise(1000 * idx)
                  .then(() => YoutubeTrack.getInfoFromVid(`https://youtube.com/watch?v=${vid}`, msg, msg.author, 0))
                  .then(video => {
                    if (limit && limit <= currentUserQueueLength + nowQueued) {
                      return;
                    }
  ...
  playNext() {
    ...
    if (this.connection) {
      ...
      let FFmpegArgs = ["-i", `${url}`, "-threads", "1", "-quality", this.premium ? "best" : "good", "-cpu-used", this.premium ? "0" : "2"];
