...
module.exports = class player {
    ...
    enqueue(msg, video) {
        YoutubeTrack.getInfoFromVid(resolveVid(msg, video), msg, (error, video)=>{
            if(error) {
                ...
                return `Enqueued ${video.prettyPrint()} currently ${this.queue.length}`;
            }
        });
    }
    ...
    playNext() {
        ...
        if(this.connection) {
            ...
            this.currentStream.on('end', () => setTimeout(this.playNext, 24000)); // 20 second leeway for bad timing
            this.connection.playRawStream(this.currentStream).then((intent) => {
                this.text.sendMessage(`Playing ${this.currentVideo.prettyPrint()}`);
                intent.on('end', this.text.sendMessage("Intent end Fired."))
            });
        }
