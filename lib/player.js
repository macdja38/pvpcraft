...
class player {
            ...
            if (!error && response.statusCode == 200) {
              ...
              body.items.forEach((elem, idx) => {
                setTimeout(()=> {
                  ...
                  promises.push(YoutubeTrack.getInfoFromVid(`https://youtube.com/watch?v=${vid}`, msg, msg.author, 0, this.raven, (error, video)=> {
                    if (limit && limit <= currentUserQueueLength + nowQueued) {
                      return;
                    }
                    if (error) {
 ...
 /*     let FFmpegArgs = ["-i", `${url}`, "-threads", "1", "-quality", this.premium ? "best" : "good", "-cpu-used",
  this.premium ? "0" : "2"];
      if (this.currentVideo.time) {
        FFmpegArgs.unshift(...["-ss", this.currentVideo.time]);
      }*/

        // fs.createReadStream("output.webm");

      /*
       let enc = {};
       enc.stdin = new EventEmitter();

       enc.stdin.pause = () => fsStream.destroy();

       this.connection.encoder.encodeStream = stream => (Promise.resolve({ stream: stream, proc: enc, channels: 2 }));
       */

      /*var thing = new WebmOpusPlayer(this.voice, {
       type: "WebmOpusPlayer",
       source: fsStream,
       });

       let stream = thing.getStream();*/
