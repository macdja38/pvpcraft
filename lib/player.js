...
class player {
                ...
                if (vid.video) {
                    YoutubeTrack.getInfoFromVid(vid.video, msg, msg.author, startTime, (error, video)=> {
                        if (error) {
                            ...
                            this.queueDB.set(this.queue, {textChannel: this.text, voiceChannel:this.voice, server: this.server});
    ...
    skipSong() {
        ...
        if(this.connection.playingIntent === null && queue.length > 0) this.playNext();
    }
