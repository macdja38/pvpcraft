...
class MusicDB extends BaseDB {
  ...
  countVideosQueuedBy(guild_id, user_id) {
    return this.r.table(this.table).get(guild_id)("queue").default([]).filter({user_id,}).count();
  }
  ...
  getBoundChannels(guilds) {
    return this.r.table(this.table).getAll(...guilds, {index: "id"}).hasFields("text_id", "voice_id").filter((doc) => {return doc("queue").default([]).count().gt(0)}).run();
  }
  ...
  clearQueue(id, options={}) {
    if (options.user_id) {
      return this.r.table(this.table).get(id).update({queue: this.r.row("queue").default([]).filter((song) => {
  ...
  getInfoFromVid(...args) {
    if (args[0].indexOf("youtu") > -1) {
      return this.fetchWithYtdl(...args).catch((err) => {
        ...
        if (errString.includes("copyright grounds") || errString.includes("your country") || errString.includes("copyright claim")) throw err;
        if (this.raven) {
