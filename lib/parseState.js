...
ParseState.prototype.getName = function getName(path) {
    try{
        if(/ Blueprint/.test(paths[path])) {
            console.log(paths[path].replace(" Blueprint", "").toLowerCase());
            return names[paths[path].replace(" Blueprint", "").toLowerCase()].value.replace("'", "") + " Blueprint";
        }
        console.log(path);
        console.log(paths[path]);
        return names[paths[path].toLowerCase()].value.replace("'", "");
    } catch (error) {
        console.error(error);
        console.error(path);
        return path;
...
ParseState.prototype.getLevel = function getName(path) {
    try{
        console.log(path);
        return names[path.toLowerCase()].value.replace("'", "");
    } catch (error) {
        console.error(error);
        console.error(path);
        return path;
