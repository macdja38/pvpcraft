...
class Permissions {
   ...
   * @param analytics
   * @param i10010n
   * @param getChannelLanguage
   */
  constructor(permsDB, analytics, i10010n, getChannelLanguage) {
    this.perms = permsDB;
      ...
      if (JSON.parse(error.response).code === 50013) {
        ...
        if (owner) {
          owner.user.getDMChannel().then(channel =>
            channel.createMessage(this.i10010n(this.getChannelLanguage(channel.id)) `Hello, I've removed and denied the permissions configuration for channel ${
