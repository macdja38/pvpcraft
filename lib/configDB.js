...
module.exports = class configDB {
  ...
  follow() {
    this.r.table(this.table).getAll(...addStar(this.client.servers.map(s => s.id))).changes().run().then((cursor)=>{
    ...
    return new Promise((resolve, reject)=> {
      try {
        this.r.tableList().contains(this.table)
          ...
          var completed = [];
          return this.r.table(this.table).getAll(...addStar(this.client.servers.map(s => s.id))).run().then(data => {
            data.forEach((thing) => {
