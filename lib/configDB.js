...
module.exports = class config {
    ...
    reload() {
        try {
            global.r.tableList().contains(this.table)
                ...
                console.log(`Connected to db ${this.table} on shard ${global.cluster.worker.id-1}`.blue);
                var completed = [];
                ...
                return Promise.all(completed).then(()=> {
                    console.log(`Loaded Data from db ${this.table} on shard ${global.cluster.worker.id-1}`.yellow);
    ...
    set(key, value, options) {
        ...
        return this.write((options && options.server) ? options.server : "*");
    };
