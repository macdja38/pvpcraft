...
module.exports = class config {
  ...
  serverCreated(server) {
    return global.r.table(this.table).get(server.id).run().then((thing)=> {
      if (thing) {
  ...
  follow() {
    global.r.table(this.table).getAll(...addStar(this.client.servers.map(s => s.id))).changes().run().then((cursor)=>{
    ...
    return new Promise((resolve, reject)=> {
      try {
        global.r.tableList().contains(this.table)
          ...
          /*global.r.table(this.table).insert([{id: "*", prefix: "//", "changeThresh": 1}]).run().then((res)=>{
           console.log(res);
           });*/
          console.log(`Connected to db ${this.table} on shard ${process.env.id}`.blue);
          var completed = [];
          return global.r.table(this.table).getAll(...addStar(this.client.servers.map(s => s.id))).run().then(data => {
            data.forEach((thing) => {
  ...
  write(options) {
    ...
    if (this.data[serverId] !== null && typeof(this.data[serverId]) === "object") {
      ...
      return global.r.table(this.table).insert(this.data[serverId], { conflict: (options.conflict) ? options.conflict : "update" }).run().catch(console.error);
    }
    return global.r.table(this.table).get(serverId).delete().run().catch(console.error);
  }
  ...
  count(key, options = {}) {
    ...
    return global.r.table(this.table).get(options.server)(key).count().run();
  }
  ...
  getRandom(key, count, options = {}) {
    ...
    return global.r.table(this.table).get(options.server)(key).sample(count).run();
  }
