...
module.exports = class config {
    ...
    reload() {
        ...
        try {
            global.r.tableList().contains(this.table)
                ...
                resolve(Promise.all(completed).then(()=> {
    ...
    add(key, value, options={}) {
        ...
        return global.r.table(this.table).insert({"id": options.server, entries: global.r.table(this.table).get(options.server)(key).setInsert(value)}, {"conflict": options.conflict}).run(this.con);
    }
