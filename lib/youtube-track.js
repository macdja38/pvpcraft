...
YoutubeTrack.getInfoFromVid = function (vid, m, user, time, cb) {
    return new Promise((resolve, reject)=> {
        ytdl.getInfo(vid, [], {maxBuffer: 1000*1024}, (err, info) => {
            if (err) {
...
YoutubeTrack.prototype.getURL = function () {
  ...
  console.log("Could not find webm for ", this.containedVideo.webpage_url);
  formats = this.containedVideo.formats
    .filter(f => f.ext === "mp4")
    .sort((a, b) => b.abr - a.abr);
  if(formats.length > 0) return (formats.find(f => f.abr > 0 && !f.resolution) || formats.find(f => f.abr > 0)).url;
  console.log("Could not find mp4 for ", this.containedVideo.webpage_url);
  formats = this.containedVideo.formats
    .filter(f => f.ext === " vorbis")
    .sort((a, b) => b.abr - a.abr);
  if(formats.length > 0) return (formats.find(f => f.abr > 0 && !f.resolution) || formats.find(f => f.abr > 0)).url;
  console.log("Could not find vorbis for ", this.containedVideo.webpage_url);
