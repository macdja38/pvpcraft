...
YoutubeTrack.prototype.getURL = function () {
  ...
  if (formats.length > 0) return (formats.find(f => (f.abr || f.audioBitrate) > 0 && !f.resolution) || formats.find(f => (f.abr || f.audioritRate) > 0)).url;
  console.log("Could not find webm for ", sourceURL);
  formats = this.containedVideo.formats
    .filter(f => f.ext === "mp4" || f.container === "mp4")
    .sort((a, b) => b.abr - a.abr);
  if (formats.length > 0) return (formats.find(f => (f.abr || f.audioritRate) > 0 && !f.resolution) || formats.find(f => (f.abr || f.audioritRate) > 0)).url;
