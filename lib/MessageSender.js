  ...
  emptyQueue() {
    for (let channelId in this._queuedMessages) {
      ...
      this._getWebhook(first.channel).then(webhook => {
        return this._client.executeSlackWebhook(webhook.id, webhook.token, options.options);
      }).catch(() => {
        if (!(first.channel instanceof Eris.GuildChannel)) return;
        let texts;
        if (queue.length > 1) {
        ...
        texts.forEach((string) => {
          first.channel.createMessage(string).catch(() => {});
        });
      })
    }
    this._queuedMessages = {};
