...
class MessageSender {
  ...
  emptyQueue() {
    for (let channelId in this._queuedMessages) {
      ...
      console.log(calledOptions.options, calledOptions.options.attachments);
      ...
      this._getWebhook(calledOptions.channel).then(webhook => {
        ...
        let texts;
        if (queue.length > 1) {
          texts = queue.slice(1).reduce((prev, cur) => `${prev.text}\n${cur.text}`, calledOptions).match(/^.{1,1999}/g);
        } else {
          texts = calledOptions.text.match(/^.{1,1999}/g);
        }
        texts.forEach((string) => {
          calledOptions.channel.createMessage(string).catch(() => {});
        });
      })
    }
    this._queuedMessages = {};
